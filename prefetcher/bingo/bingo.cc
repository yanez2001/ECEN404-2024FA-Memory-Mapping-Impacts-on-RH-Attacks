    ...
    int prefetch(CACHE *cache, uint64_t block_address) {
        if (this->debug_level >= 2) {
            cerr << "PrefetchStreamer::prefetch(cache=" << cache->NAME << ", block_address=0x" << hex << block_address
                 << ")" << dec << endl;
            cerr << "[PrefetchStreamer::prefetch] " << cache->get_occupancy(3, block_address) << "/" << cache->get_size(3, block_address)
                 << " PQ entries occupied." << dec << endl;
            cerr << "[PrefetchStreamer::prefetch] " << cache->get_occupancy(0, block_address) << "/" << cache->get_size(0, block_address)
        ...
        for (int d = 1; d < this->pattern_len; d += 1) {
            ...
            for (int sgn = +1; sgn >= -1; sgn -= 2) {
                ...
                if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0) {
                    ...
                    if (cache->get_occupancy(3, pf_address) + cache->get_occupancy(0, pf_address) < cache->get_size(0, pf_address) - 1 &&
                        cache->get_occupancy(3, pf_address) < cache->get_size(3, pf_address)) {
...
class Bingo {
  ...
  private:
     ...
     */
    vector<bool> vote(const vector<vector<bool>> &x) {
        if (this->debug_level >= 2)
        ...
        bool pf_flag = false;
        vector<bool> res(this->pattern_len, false);
        for (int i = 0; i < n; i += 1)
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in){
    ...
    if (type != (uint8_t)access_type::LOAD)
        return metadata_in;
