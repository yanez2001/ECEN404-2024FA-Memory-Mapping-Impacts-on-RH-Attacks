...
{ 
  ...
  num_on_time = historyt[cpu]->get(latency, tag, line_addr, tags, addr, cycle);
...
void CACHE::prefetcher_initialize() 
{
  ...
  scache.push_back(new ShadowCache(this->NUM_SET, this->NUM_WAY));
  historyt.push_back(new HistoryTable());
  berti.push_back(new Berti());
...
if (type == (uint8_t)access_type::PREFETCH) return metadata_in; // there are instruction prefetches with my stuff
  ...
  } else if (cache_hit && scache[cpu]->is_pf(line_addr)) // Hit bc prefetch
  {
    ...
    scache[cpu]->set_pf(line_addr, false);
    // Get latency
    uint64_t latency = scache[cpu]->get_latency(line_addr);
  ...
  for (int i = 0; i < BERTI_TABLE_DELTA_SIZE; i++)
  {
    ...
    if (latencyt[cpu]->get(p_b_addr)) continue;
...
{
  uint64_t line_addr = (addr >> LOG2_BLOCK_SIZE); // Line addr
  uint64_t tag     = latencyt[cpu]->get_tag(line_addr);
  uint64_t cycle   = latencyt[cpu]->del(line_addr) & TIME_MASK;
  uint64_t latency = 0;
  ...
  scache[cpu]->add(set, way, line_addr, prefetch, latency);

  if (latency != 0 && !prefetch)
  {
    berti[cpu]->find_and_update(latency, tag, cycle, line_addr, cpu);
  }
  return metadata_in;
}
