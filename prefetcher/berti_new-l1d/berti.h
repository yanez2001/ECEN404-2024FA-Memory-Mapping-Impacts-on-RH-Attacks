...
welford_t average_latency_L1D;

// Get more info
uint64_t pf_to_l1_L1D = 0;
uint64_t pf_to_l2_L1D = 0;
uint64_t pf_to_l2_bc_mshr_L1D = 0;
uint64_t cant_track_latency_L1D = 0;
uint64_t cross_page_L1D = 0;
uint64_t no_cross_page_L1D = 0;
uint64_t no_found_berti_L1D = 0;
uint64_t found_berti_L1D = 0;
...
class LatencyTable_L1D
{
  ...
  public:
    LatencyTable_L1D(const int size) : size(size)
    {
      latencyt = new latency_table[size];
    }
    ~LatencyTable_L1D() { delete latencyt;}
...
class ShadowCache_L1D
{
  ...
  public:
    ShadowCache_L1D(const int sets, const int ways)
    {
...
std::vector<LatencyTable_L1D *>latencyt_L1D;
std::vector<ShadowCache_L1D *>scache_L1D;
std::vector<HistoryTable_L1D *>historyt_L1D;
std::vector<Berti_L1D *>berti_L1D;
#endif
